...
std::size_t find_crlf(const std::vector<uint8_t> &buf, std::size_t start) {
...
ParseResult parse_command(const std::vector<uint8_t> &buf) {
  ...
  for (std::size_t i = 1; i < crlf_index; ++i) {
    ...
    const std::size_t digit = static_cast<std::size_t>(buf[i] - '0');

    if (num_elements > (std::numeric_limits<std::size_t>::max() - digit) / 10) {
      result.status = ParseStatus::Invalid;
      return result;
    }
  ...
  for (std::size_t element_index = 0; element_index < num_elements;
    ...
    const std::size_t length_crlf_index = find_crlf(buf, pos);
    ...
    for (std::size_t i = pos + 1; i < length_crlf_index; ++i) {
      if (buf[i] < '0' || buf[i] > '9') {
        result.status = ParseStatus::Invalid;
        return result;
      }
    ...
    // Move past "$<length>\r\n".
    const std::size_t string_start = length_crlf_index + 2;
    ...
    const std::size_t string_end = string_start + string_length;

    // Check whether the complete payload and trailing CRLF exist.
    if (string_end > buf.size() || buf.size() - string_end < 2) {
      result.status = ParseStatus::Incomplete;
      return result;
    }

    if (buf[string_end] != '\r' || buf[string_end + 1] != '\n') {
      result.status = ParseStatus::Invalid;
      return result;
    }

    result.args.emplace_back(reinterpret_cast<const char *>(&buf[string_start]),
                             string_length);
